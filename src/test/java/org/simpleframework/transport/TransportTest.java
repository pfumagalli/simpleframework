...
public class TransportTest {
    ...
    private void testTransport(int repeat) throws Exception {
    ...
    private void testTransport(int size, int repeat) throws Exception {
        ...
        final SocketConsumer consumer = new SocketConsumer(size, repeat);
        final SocketAddress address = new InetSocketAddress("localhost", consumer.getPort());
        ...
        final MockSocket pipeline = new MockSocket(channel);
        final Transport transport = new SocketTransport(pipeline, reactor, 8192);
        final OutputStream out = new TransportOutputStream(transport);
    ...
    private void testSocket(int size, int repeat) throws Exception {
        // ThreadDumper dumper = new ThreadDumper();
        final SocketConsumer consumer = new SocketConsumer(size, repeat);
        final Socket socket = new Socket("localhost", consumer.getPort());
    ...
    private class AlpahbetIterator {
        ...
        private final byte[] alphabet = "abcdefghijklmnopqstuvwxyz".getBytes();
    ...
    private void testOutputStream(SocketConsumer consumer, OutputStream out, int size, int repeat) throws Exception {
        final byte[] block = new byte[size]; // write size
        final AlpahbetIterator it = new AlpahbetIterator(); // write known data

        for(int i = 1; i < block.length; i++) {
            block[i] = it.next();
        }
        final AtomicLong count = new AtomicLong();
        final PerformanceMonitor monitor = new PerformanceMonitor(consumer, count, out.getClass().getSimpleName(), size);
    ...
    private class PerformanceMonitor extends Thread {
        ...
        public void run() {
            ...
            while(!dead) {
                try {
                    final long octets = count.longValue();
    ...
    private class SocketConsumer extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                final InputStream source = new BufferedInputStream(in);
                final AlpahbetIterator it = new AlpahbetIterator();
    ...
    private static class TransportOutputStream extends OutputStream {
        ...
        public void write(int octet) throws IOException {
            final byte[] data = new byte[] { (byte) octet };
            write(data);
        }
        ...
        public void write(byte[] data, int off, int len) throws IOException {
            try {
                final ByteBuffer buffer = ByteBuffer.wrap(data, off, len);
                final ByteBuffer safe = buffer.asReadOnlyBuffer();
