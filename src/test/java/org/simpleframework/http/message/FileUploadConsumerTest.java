...
public class FileUploadConsumerTest {
    ...
    public void testNoFinalCRLF() throws Exception {
        final byte[] data = SOURCE.getBytes("UTF-8");
        final byte[] boundary = "mxvercagiykxaqsdvrfabfhfpaseejrg".getBytes("UTF-8");
        final Allocator allocator = new ArrayAllocator();
        final FileUploadConsumer consumer = new FileUploadConsumer(allocator, boundary, data.length);
        final ByteCursor cursor = new StreamCursor(data);

        while(!consumer.isFinished()) {
            consumer.consume(cursor);
        }
        AssertJUnit.assertEquals(consumer.getBody().getContent(), SOURCE);
        AssertJUnit.assertEquals(consumer.getBody().getParts().size(), 6);
    }
    ...
    public void testNoFinalCRLSWithDribble() throws Exception {
        ...
        final FileUploadConsumer consumer = new FileUploadConsumer(allocator, boundary, data.length);
        final ByteCursor cursor = new StreamCursor(data);
        final DribbleCursor dribble = new DribbleCursor(cursor, 1);
    ...
    public void testNoFinalCRLSWithDribble3() throws Exception {
        ...
        final FileUploadConsumer consumer = new FileUploadConsumer(allocator, boundary, data.length);
        final ByteCursor cursor = new StreamCursor(data);
        final DribbleCursor dribble = new DribbleCursor(cursor, 3);
