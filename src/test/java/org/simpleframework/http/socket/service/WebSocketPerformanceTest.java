...
public class WebSocketPerformanceTest {
    ...
    public static class MessageGeneratorService extends Thread implements Service {
        ...
        public void distribute(Frame frame) {
            try {
                for(final FrameChannel socket : sockets) {
                    try {
                        socket.send(frame);
                    } catch(final Exception e){
                        e.printStackTrace();
    ...
    public static class MessageGeneratorClient extends Thread {
        ...
        public void run() {
            try {
                ...
                final StreamCursor cursor = new StreamCursor(socket.getInputStream());
                final FrameConsumer consumer = new FrameConsumer();
                final ReplyConsumer response = new ReplyConsumer();

                final byte[] request = ("GET /chat HTTP/1.1\r\n"+
                ...
                while(cursor.isOpen()) {
                    ...
                    if(consumer.isFinished()) {
                        final Frame frame = consumer.getFrame();
        ...
        public void validate(Frame frame) throws Exception {
            ...
            if(type == FrameType.TEXT) {
                ...
                final String time = text.substring(0, index);
                final long sendTime = Long.parseLong(time);
                final long timeElapsed = System.currentTimeMillis() - sendTime;
        ...
        public void run() {
            while(true) {
                try {
                    ...
                    final long total = duration.getAndSet(0);
                    final long average = (total > 0 ? total : 1) / (count > 0 ? count : 1);
    ...
    public static class ConsoleAnalyzer extends Daemon implements TraceAnalyzer {
        ...
        public void run() {
            try {
                while(isActive()) {
                    ...
                    while(!queue.isEmpty()) {
                        final TraceRecord record = queue.poll();

                        if(filter != null) {
                            final Object event = record.event;
        ...
        private class TraceFeeder implements Trace {
            ...
            public void trace(Object event, Object value) {
                final TraceRecord record = new TraceRecord(channel, event, value, sequence);
        ...
        private class TraceRecord {
            ...
            public String toString() {
                ...
                final PrintWriter writer = new PrintWriter(builder);
    ...
    public static void main(String[] list) throws Exception {
        ...
        final ConsoleAnalyzer agent = new ConsoleAnalyzer();
        final AtomicLong duration = new AtomicLong();
        final AtomicInteger counter = new AtomicInteger();
        final MessageGeneratorService service = new MessageGeneratorService();
        final MessageGeneratorContainer container = new MessageGeneratorContainer(service, agent, 7070);
        final MessageTimer timer = new MessageTimer(counter, duration);
        ...
        for(int i = 0; i < 100; i++) {
            final MessageGeneratorClient client = new MessageGeneratorClient(service, counter, duration, 7070, false);
            client.start();
        }
    }
}
