...
public class WebSocketTableSweeper {
    ...
    public Map<WebSocketTableUpdateType, String> sweep(long time, long count) {
        final Map<WebSocketTableUpdateType, String> messages = new LinkedHashMap<WebSocketTableUpdateType, String>();

        if(count <= 1) {
            final WebSocketTableSchema schema = table.getSchema();
            final String schemaUpdate = schema.createStyle();
