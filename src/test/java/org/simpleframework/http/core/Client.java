...
public class Client {
    ...
    static {
        try {
            ...
            final KeyManagerFactory keyFactory = KeyManagerFactory.getInstance("SunX509");
            sslContext = SSLContext.getInstance("TLS");//SSLv3");
            final InputStream stream = new ByteArrayInputStream(CERTIFICATE);
            final X509TrustManager trustManager = new AnonymousTrustManager();
            final X509TrustManager[] trustManagers = new X509TrustManager[]{trustManager};
    ...
    public static void main(String[] list) throws Exception {
        final FileOutputStream out = new FileOutputStream("c:\\client");
        final PrintStream console = System.out;
        final OutputStream dup = new FilterOutputStream(out) {
            @Override
        ...
        final PrintStream p = new PrintStream(dup, true);

        System.setOut(p);
        System.setErr(p);
        final Socket socket = factory.createSocket("localhost", 9091);
